...
async function bypassLootLabs(rawUrl, log, preHtml) {
  log(`[lootlabs] target: ${rawUrl.slice(0, 100)}`);
  let html = (preHtml || "").trim();
  let cookies = "";
  if (!html) {
    try {
    ...
    log(`[lootlabs] using pre-fetched HTML (${html.length} bytes)`);
  }
...
async function bypassLootLabsFromHtml(rawUrl, html, log) {
  return bypassLootLabs(rawUrl, log, html);
}
export {
  bypassLootLink,
  bypassLootLabs,
  bypassLootLabsFromHtml,
  bypassWorkInk
};
